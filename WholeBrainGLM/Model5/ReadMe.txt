...
This model is designed to assess the differences between the three tasks. It should be compared to model 3 (same as this but it used the measured distace for value) and model 1 (same as this but uses the max-likelihood estimate as Value).

Value Model -- For this model the value of the options was estomated by a max-likelihood model that used all tasks ["CSBValue"] (model1 only used the control and scaling tasks). But it did not use any dumbies to caputre externality terms. 
