...
This model is designed to assess the differences between the three tasks. It should be compared to model 5 (same as this but does not use a dumby for the value of scaling and bundling) and model 7 (same as this but uses a parametric dumby for the value of scaling and bundling).

Value Model -- For this model the value of the options was estomated by a max-likelihood model that used all tasks ["CSBValue"] (model1 only used the control and scaling tasks). It also includes a fixed dumby for bunding and a different fixed dumby for scaling). 
