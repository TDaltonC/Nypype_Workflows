...
#   Add a column of ones to the dataframe (this is usefull for creating the three column files)    
    ...
    trialbytrial['linearValue'] = np.log(optionValue['optionValue']-np.min(optionValue['optionValue'])+.001)
    trialbytrial['linearValue'] = trialbytrial['linearValue'] - np.mean(trialbytrial['linearValue'])
...
#   Fliter down to multi-run event files  
    ...
    difficultyTrials = trialbytrial[(trialbytrial.ltrialType  != 1)]
